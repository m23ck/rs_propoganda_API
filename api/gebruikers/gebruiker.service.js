...
module.exports = {
    ...
    getGebruikerById: (gebruiker_id, callBack) => {
        pool.query(`select gebruiker_id, naam, voornaam, adres, telefoonnummer, email, gebruikernaam, wachtwoord, type_id from gebruikers where id = ?`,
            [gebruiker_id],
            (error, results, fields) => {
                if(error){
                    return callBack(error);
    ...
    updateGebruiker: (data, callBack) => {
        pool.query(`update gebruikers set naam = ?, voornaam = ?, adres = ?, telefoonnummer = ?, email = ?, gebruikernaam = ?, wachtwoord = ?, type_id = ? where id = ?`,
            [
                data.naam,
                data.voornaam,
                data.adres,
